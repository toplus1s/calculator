...
QString calculator::revalue(int e, double x)
{
    if(e == 1)
        return QString ::number(ValueofX(Aa,x));
    else if(e == 2)
        return QString ::number(ValueofX(Bb,x));
    else
        return QString ::number(ValueofX(Cc,x));
}



double calculator::ValueofX(polynomial *X, double x)
{
    double sum=0;
    polynomial *a=X;
    while(a->next)
